...
const API_BASE = 'https://api-six-gamma-99.vercel.app';
...
function App() {
  ...
  const discoverKeywords = async () => {
    ...
    try {
      ...
      setStatus({ type: 'error', message: err.response?.data?.error || 'Failed to discover keywords' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const generateArticle = async (keyword) => {
    ...
    try {
      ...
      setStatus({ type: 'error', message: err.response?.data?.error || 'Failed to generate article' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const createPreview = async () => {
    ...
    try {
      ...
      setStatus({ type: 'error', message: err.response?.data?.error || 'Failed to create preview' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const publishArticle = async () => {
    ...
    try {
      ...
      await axios.post(API_BASE, { post_id: parseInt(postId) });
