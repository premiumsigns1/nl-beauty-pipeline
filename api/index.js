...
function addAffiliateLinks(content, keyword) {
  ...
  const disclaimer = '<p><em>*This blog may contain affiliated links and discount codes.</em></p>';
  const firstPara = content.indexOf('</p>');
  if (firstPara > 0) {
    content = content.slice(0, firstPara + 4) + ' <a href="' + links[0].url + '"><em>' + links[0].anchor + '</em></a>' + content.slice(firstPara + 4);
  }
