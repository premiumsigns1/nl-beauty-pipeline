...
module.exports = async (req, res) => {
  ...
  if (path === '/keywords') {
    ...
    const keywords = [
      ...
      { keyword: `${topic} 2026`, url: '#', snippet: 'Latest' },
  ...
  if (path === '/save-article') {
    ...
    const article = { id: articleId, keyword, title, meta_description: meta_description || '', content: contentWithLinks, status: 'draft', affiliate_links: selectAffiliateLinks(keyword) };
    articlesDb[articleId] = article;
    return res.status(200).json({ article_id: articleId, title, preview: { title, content: contentWithLinks.slice(0, 500) + '...', affiliate_links: article.affiliate_links } });
  }
  ...
  if (path === '/preview') {
    ...
    if (!article) return res.status(404).json({ error: 'Not found' });
    return res.status(200).json({ article_id: article.id, title: article.title, content: article.content, schema: JSON.stringify({ '@context': 'https://schema.org', '@type': 'Article', headline: article.title }) });
  }
  ...
  if (path === '/publish') {
    ...
    try {
      ...
      const resp = await axios.post(`https://api.webflow.com/collections/${WEBFLOW_COLLECTION_ID}/items`, { fields: { name: article.title, slug, content: article.content, description: article.meta_description } }, { headers: { 'Authorization': `Bearer ${WEBFLOW_API_TOKEN}`, 'Content-Type': 'application/json' } });
